...
const EXACT_VI = new Map(Object.entries({
  ...
  'Create credential': 'Tạo credential',
  Name: 'Tên',
  ...
  APPEND: 'THÊM DÒNG',
  'None.': 'Không có.',
  'Node inspector': 'Bảng cấu hình node',
  'Close node inspector': 'Đóng bảng cấu hình node',
  'Not run': 'Chưa chạy',
  'No execution selected': 'Chưa chọn lần chạy',
  'Inspector tabs': 'Các tab cấu hình node',
  Parameters: 'Tham số',
  Input: 'Đầu vào',
  Logs: 'Nhật ký',
  'Node failed': 'Node chạy thất bại',
  'No error details were reported for this node.': 'Node không trả về chi tiết lỗi.',
  'Node Name': 'Tên node',
  'Required parameters': 'Tham số bắt buộc',
  'Common parameters': 'Tham số thường dùng',
  credential: 'Credential',
  resource: 'Tài nguyên',
  operation: 'Thao tác',
  advanced: 'Nâng cao',
  'Field value mode': 'Chế độ giá trị trường',
  Fixed: 'Giá trị cố định',
  Expression: 'Biểu thức',
  '-- Select Credential Secret --': '-- Chọn credential --',
  'Format JSON': 'Định dạng JSON',
  'Pick data': 'Chọn dữ liệu',
  'Expression preview': 'Xem trước biểu thức',
  'No expression selected': 'Chưa chọn biểu thức',
  'Choose a data path below.': 'Chọn một đường dẫn dữ liệu bên dưới.',
  'Convert to JSON literal': 'Chuyển thành JSON cố định',
  'Hide advanced options': 'Ẩn tùy chọn nâng cao',
  'Show advanced options': 'Hiện tùy chọn nâng cao',
  'No configurable parameters for this node.': 'Node này không có tham số cần cấu hình.',
  'Data picker': 'Chọn dữ liệu',
  'Source node selector': 'Chọn node nguồn',
  'Direct input': 'Đầu vào trực tiếp',
  'Previous node': 'Node trước',
  'Trigger payload': 'Dữ liệu trigger',
  'Search input data': 'Tìm trong dữ liệu đầu vào',
  'Search paths or values...': 'Tìm đường dẫn hoặc giá trị...',
  'AI Quick Config': 'AI cấu hình nhanh',
  "Ask AI to fill this node's parameters.": 'Yêu cầu AI điền tham số cho node này.',
  'Describe how to configure this node...': 'Mô tả cách bạn muốn cấu hình node...',
  'Hide Node Guide': 'Ẩn hướng dẫn node',
  'Show Node Guide': 'Hiện hướng dẫn node',
  'Delete Node': 'Xóa node',
  'No input data': 'Chưa có dữ liệu đầu vào',
  'No input data is available. Run this workflow first, or connect an upstream node that produces output.': 'Chưa có dữ liệu đầu vào. Hãy chạy workflow hoặc nối một node phía trước có output.',
  'JSON tree': 'Cây JSON',
  Table: 'Bảng',
  'Raw JSON': 'JSON thô',
  'Search input...': 'Tìm trong đầu vào...',
  'No output data': 'Chưa có dữ liệu đầu ra',
  'Copy output': 'Sao chép output',
  'Download output': 'Tải output',
  'Copy failed': 'Sao chép thất bại',
  Copied: 'Đã sao chép',
  'Template Gallery': 'Thư viện mẫu workflow',
  'Use Template': 'Dùng mẫu này',
  All: 'Tất cả',
  Beginner: 'Cơ bản',
...
export function translateOperatorText(value) {
  ...
  match = trimmed.match(/^Source:\s*(.+)$/i);
  if (match) return `Nguồn: ${match[1]}`;
  match = trimmed.match(/^Click a value to insert an expression into\s+(.+)\.$/i);
  if (match) return `Bấm một giá trị để chèn biểu thức vào ${match[1]}.`;
  match = trimmed.match(/^(\d+) nodes?$/i);
  if (match) return `${match[1]} node`;
  match = trimmed.match(/^(\d+) edges?$/i);
  if (match) return `${match[1]} dây nối`;
  return value;
}
