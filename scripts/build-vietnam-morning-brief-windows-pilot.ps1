...
$ArtifactDirectoryName = 'Goflow-Vietnam-Morning-Brief-Windows-amd64'
$BundleFileName = 'Goflow-Vietnam-Morning-Brief-Windows-amd64.zip'
$MarkerFileName = 'UNSIGNED-PERSONAL-PILOT.txt'
$ChecksumFileName = 'SHA256SUMS.txt'
$PackDirectory = 'examples/packs/vietnam-morning-brief'
$PackID = 'official.vietnam-morning-brief'
$PackVersion = '0.1.0'
    ...
    foreach ($root in $Roots) {
        foreach ($file in Get-ChildItem -LiteralPath $root -Recurse -File) {
            ...
            foreach ($value in $values) {
                foreach ($variant in @($value, $value.Replace('\', '/')) | Select-Object -Unique) {
...
if ($env:OS -ne 'Windows_NT') {
    throw 'Vietnam Morning Brief Windows pilot artifacts must be built natively on Windows'
}

$RepoRoot = [System.IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..'))
$OutputDirectory = [System.IO.Path]::GetFullPath($OutputDirectory)
$TempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("goflow-vietnam-morning-brief-pilot-" + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    Invoke-Checked -Command $RuntimePath -Arguments @('pack', 'validate', $PackDirectory)
    Invoke-Checked -Command $RuntimePath -Arguments @('pack', 'test', $PackDirectory, '--output', 'json') | Out-Null
    ...
    if ($BundleDigest -ne $SecondDigest) {
        throw 'native Windows Vietnam Morning Brief bundle build is not deterministic'
    }
    ...
    $manifestPath = Join-Path $Extracted 'pack\pack.json'
    $workflowPath = Join-Path $Extracted 'pack\workflows\main.json'
    $runtimeInBundle = Join-Path $Extracted 'goflow.exe'
    foreach ($requiredPath in @($packInfoPath, $manifestPath, $workflowPath, $runtimeInBundle)) {
        if (-not (Test-Path -LiteralPath $requiredPath -PathType Leaf)) {
            throw "portable bundle is missing required file: $requiredPath"
        }
    }

    $manifest = Get-Content -LiteralPath $manifestPath -Raw | ConvertFrom-Json
    if ($manifest.id -ne $PackID -or $manifest.version -ne $PackVersion) {
        throw "portable bundle contains unexpected pack identity: $($manifest.id)@$($manifest.version)"
    }
    ...
    Copy-Item -LiteralPath (Join-Path $RepoRoot 'docs\VIETNAM_MORNING_BRIEF_WINDOWS_PILOT.md') -Destination (Join-Path $OutputDirectory 'START_HERE.md')

    $MarkerPath = Join-Path $OutputDirectory $MarkerFileName
    @(
        'UNSIGNED-PERSONAL-PILOT'
        "commit=$Commit"
