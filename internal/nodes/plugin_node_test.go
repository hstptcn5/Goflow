...
func TestGoflowPluginExecutorOffline(t *testing.T) {
	...
	nodeEmpty := &Node{Params: map[string]interface{}{"plugin_name": ""}}
	...
	// Test 2: Execution failure for missing plugin or missing plugins directory.
	nodeOffline := &Node{Params: map[string]interface{}{"plugin_name": "non_existent_plugin_filename_12345"}}
	_, err = executor.Execute(ctx, nodeOffline)
	if err == nil || !strings.Contains(strings.ToLower(err.Error()), "plugin") {
		t.Errorf("Expected plugin resolution error, got: %v", err)
	}

	// Test 3: Validation rejects path traversal.
	nodeTraversal := &Node{Params: map[string]interface{}{"plugin_name": "../outside"}}
