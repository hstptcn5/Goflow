...
func TestPostgresQueryExecutorOffline(t *testing.T) {
	...
	// Test 1: Empty connection string/credential error.
	nodeEmptyConn := &Node{Params: map[string]interface{}{
		"connection_string": "",
		"query":             "SELECT 1;",
		"query_type":        "SELECT",
	}}
	_, err := executor.Execute(ctx, nodeEmptyConn)
	if err == nil || !strings.Contains(err.Error(), "connection string or encrypted credential is required") {
		t.Errorf("Expected missing PostgreSQL connection error, got: %v", err)
	}

	// Test 2: Empty query error.
	nodeEmptyQuery := &Node{Params: map[string]interface{}{
	...
	// Test 3: Connection failure error on an offline port.
	nodeOffline := &Node{Params: map[string]interface{}{
